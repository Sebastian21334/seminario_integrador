...
export class AdvertiserProfileComponent {
  ...
  protected readonly locations = computed(() => Array.from(new Set(this.publications().map((item) => [item.ciudad?.nombre, item.provincia?.nombre].filter(Boolean).join(', ')).filter(Boolean))).join(' · '));

  constructor() {
    const id = Number(this.route.snapshot.paramMap.get('id'));
    if (!Number.isInteger(id) || id < 1) { this.error.set('El perfil solicitado no es válido.'); this.loading.set(false); return; }
    this.listings.getByAdvertiser(id).pipe(finalize(() => this.loading.set(false))).subscribe({
      next: (items) => { this.publications.set(items); if (!items.length) this.error.set('Este anunciante no tiene publicaciones activas.'); },
      error: () => this.error.set('No pudimos cargar este perfil.'),
    });
  }
  protected whatsappUrl(): string | null {
    const phone = this.advertiser()?.numero_contacto || this.advertiser()?.usuario?.telefono;
